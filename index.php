...
<section>
  ...
  <form action="products.php" method="get">
    <select name="cat">
      ...
      <?php
      //category distinct select korbo
...
<?php if(!empty($_SESSION['user_id']) && $_SESSION['user_type'] === 'Seller'): ?>
<section>
  <h2>Seller Options</h2>
  <ul>
    <li><a href="selle_upload.php">➕ Add Products</a></li>
  </ul>
</section>
<?php endif; ?>
...
<section>
  <h3>Quiz (SDG)</h3>
  <p><a href="quiz.php">Take SDG Quiz → Get Reward Discount</a></p>
</section>
...
<section class="orders">
  ...
  <?php
    ...
    //recent orders nibo join korbo order r places er table theke user er order ta anbo
    $orders = $pdo->prepare("SELECT o.OrderID, o.Order_date, o.Status, o.TotalAmount 
                             FROM `order` o
                             JOIN places p ON o.OrderID = p.OrderID
                             WHERE p.UserID = ?
                             GROUP BY o.OrderID
  ...
  ?>
    ...
    <?php foreach($ordersList as $order): ?>
      <li>
        <span>Order #<?= $order['OrderID'] ?> - <?= htmlspecialchars($order['Status']) ?> (<?= $order['Order_date'] ?>)</span>
        <a href="order_memo.php?order=<?= $order['OrderID'] ?>">View Memo</a>
      </li>
    <?php endforeach; ?>
    </ul>
  <?php else: ?>
